 ...
 * 1. wttp-gateway (public package)
 * 2. @wttp/gateway (organization scoped package)
 */
...
function main(): void {
  ...
  try {
    ...
    publishPackage('wttp-gateway', 'package.json', options);
    
    // Publish organization scoped package (temporarily swaps package.json)
    publishPackage('@wttp/gateway', 'package.wttp.json', options);
    
    console.log('\n🎉 All packages published successfully!');
    
    if (!dryRun) {
      console.log('\n📋 Installation commands:');
      console.log('npm install wttp-gateway');
      console.log('npm install @wttp/gateway');
    }
